...
public class AdaBoost extends Classifier {
    // 存储可放回抽取生成的新样本集
    class RepickSamples implements Serializable {
        double[][] features;
        double[] labels;
        int[] index;
    }
    ...
    // 学习结果
    class LearningResult implements Serializable{
        double error; // 训练错误率
        boolean correct[];
    }
    ...
    @Override
    public void train(boolean[] isCategory, int classifier,double[][] features, double[] labels) {
        classifiers = new Classifier[classifier];//九个分类器
        weightsOfClassifiers = new double[classifier];//每个分类器的权重
        
        int samplesCount = labels.length;//样本个数
        weightsOfSamples = new double[samplesCount];//每个样本的权重
        
        for (int i = 0; i < samplesCount; ++i) {
            weightsOfSamples[i] = (double)1 / samplesCount;//样本权重取平均
        }
        ...
        for (int i = 0; i < classifier; ++i) {
            ...
            do {
                samples = repickSamplesWithWeights(features, labels);//错误率大于0.5时 重选样本
                classifiers[i] = new DecisionTree();                 //新建一颗分类器决策树
                classifiers[i].train(isCategory, classifier, samples.features, samples.labels);//训练样本 这个isCategory应该是用来去掉非分类数据
                
                result = validateLearningPerformance(classifiers[i], samples);//验证学习效果
            } while (result.error > 0.5);
            
            // 调整样本权重
            weightsOfClassifiers[i] = 0.5 * Math.log((1 - result.error) / result.error);//错误率目前低于0.5 给错误率低的分类器更高的权重
            HashSet<Integer> checker = new HashSet<Integer>();
            for (int j = 0; j < result.correct.length; ++j) {
                //同一样本不重复更新权值
                if (checker.contains(samples.index[j])) continue;//去重
...
class DecisionTree extends Classifier {
    ...
    // 存储分割信息
    class SplitData implements Serializable {
        int split_attr;
        double[] split_points;
        int[][] split_sets;       // 分割后新的样本集合的数组
    }

    class BundleData implements Serializable {
        double floatValue;        // 存储增益率或MSE
        SplitData split_info;
    }

    // 当分割出现错误时抛出此异常
    class SplitException extends Exception implements Serializable {
    }
